...
I'm not sure yet if this would result in removing plots from use but we would need to know:
1. Was there a silvicultural treatment?
2. Was the plot planted?
3. How old is the plot?
4. What size classes of trees were measured?
5. Is it a fixed area or variable area plot?

```{r}
criteria.samples[,bgc_ss_grd]
criteria.samples[,sampletype]
criteria.samples[,treatment]
criteria.samples[,plot_typ]
criteria.samples[,area_pm]
criteria.samples[,no_plots]
criteria.samples[,no_meas]
criteria.samples[,meas_yr_first]
criteria.samples[,meas_yr_last]
criteria.samples[,dbhlimit_breakpt] #tag limit in main vs. subplots
criteria.samples[,dbhlimit_tag] #tag limit in sub-plots
criteria.samples[which(criteria.samples[,bgc_ss_grd==2]),SAMP_ID]
