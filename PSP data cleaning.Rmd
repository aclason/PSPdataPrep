...
criteria.samples <- uni.samples.dt[bgc_zone=="SBS"]
...
plot.SORTIE <- unique(remeas.samples[bgc_ss_grd==2]$SAMP_ID)
...
tree.dat.plot.SORTIE[which(tree.dat.plot.SORTIE[,species=="DM"]),.(samp_id)]

#how many trees of each species by plot
tree.per.plot <- tree.dat.plot.SORTIE[,.N, by=.(samp_id,species)]
#based on this, the only 02 plot I would consider dropping across all SBS zones included here would be plot 59016, but still <10% other species. Move forward for now with all species included.
