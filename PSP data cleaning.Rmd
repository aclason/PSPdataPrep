...
2. Work through any other criteria for plot inclusion (might have to be Alana that does that?)
3. Create SORTIE starting conditions for each plot that meets the criteria
4. Somewhere need to know how many years we need to run sortie for each PSP
5. Export those csvs for use with makeFiles in the SORTIEparams repo


I think this .RMD is just about trying to figure out which PSP plots meet our criteria for use in a study. I haven't gone through it closely, but I think I read in all the data and then figured out which ones met the criteria. It will be common across different projects that we want to select PSPs based on their BEC classification. I'm wondering if we could make a function that only imports the sample data first (that tells us site details about the plot, but not individual tree measurements), and select the plots that are within the BEC zone of interest? I think having to specify the TSA ids would be nice to get rid of. Can we just look through all the sample csvs that are in a BEC xone of interest without specifying tsas?

The sel.psp function in SORTIE-HelperFunctions.R seems like it might do this already?

A lot of the names have . in them...I don't use that much anymore, as I think it's harder to read. 

We won't need all of the functions in the SORTIE-HelperFunctions.R file, I just put it in here in case there are some that are useful. I haven't done the roxygen documentation for those functions yet. I also haven't worked on those functions in a few years, so very likely better ways of doing things.
