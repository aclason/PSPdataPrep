...
Tree.species <- c("Interior_Spruce","Lodgepole_Pine","Subalpine_Fir","Trembling_Aspen")
Min.Adult.DBH <- c(3.0,5.0,5.0,5.0)
Max.Seedling.Hgt.m <- rep(1.35, length(Tree.species))
Init.Dens.Seedling.Hgt.Class.1 <- rep(0, length(Tree.species))
Init.Dens.Seedling.Hgt.Class.2 <- rep(0, length(Tree.species))
Init.Dens.Seedling.Hgt.Class.3 <- rep(0, length(Tree.species))
Init.Dens.Seedling <- rep(0, length(Tree.species))
sizeClasses <- seq(4,80, by =2)
  inits <- vector()
  init.values <- matrix(nrow=length(sizeClasses),ncol=length(Tree.species))
...
colnames(SORTIE.tree.dat)<-Tree.species

# Figure out which tree values to change (all in #/ha):
tree.dat <- dat.list[[2]]
P60136.G000004 <- tree.dat[samp_id=="60136 G000004" & meas_no==0]
...
P60136.G000004[,LD_Group:=ifelse(ld=="L",1,ifelse(ld=="I",1,ifelse(ld=="V",1,2)))]
red.P60136.G000004 <- P60136.G000004[,.(samp_id,meas_yr,phf_tree,species,dbh,ld,LD_Group,age_bh,height)]

#just live species
ld.red.P60136.G000004 <- red.P60136.G000004[LD_Group==1]
for(i in 1:length(sizeClasses)){
    ld.red.P60136.G000004[dbh <= sizeClasses[i] & dbh > sizeClasses[i-1], DBH_bin := i]
}

tree.per.bin <- ld.red.P60136.G000004[,.N, by=DBH_bin]
tree.per.bin[,Trees.per.ha := N*12.34125]
setkey(tree.per.bin,DBH_bin)

sizeClasses[tree.per.bin[[1]]]
